...
parser=argparse.ArgumentParser(prog='funannotate-p2d.py',
    description='''Funannotate script to run tblastn/exonerate protein2dna.''',
    epilog="""Modified from funannotate-p2g.py written by Jon Palmer (2016) nextgenusfs@gmail.com""",
...
parser.add_argument('--logfile', default='funannotate-p2d.log', help='logfile')
...
def runDiamond(input, query, cpus, output):
    ...
    # now run search
    cmd = ['diamond', 'blastx', '--threads', str(cpus), '-q', input, '--db', 'diamond', '-o', 'diamond.matches.tab',
           '-e', '1e-10', '-k', '0', '--more-sensitive', '-f', '6', 'sseqid', 'slen', 'sstart', 'send', 'qseqid',
...
def runtblastn(input, query, cpus, output, maxhits):
    ...
    cmd = ['dustmasker', '-in', input, '-infmt', 'fasta', '-parse_seqids', '-outfmt', 'maskinfo_asn1_bin', '-out',
           'genome_dust.asnb']
    lib.runSubprocess(cmd, output, lib.log)
    cmd = ['makeblastdb', '-in', input, '-dbtype', 'nucl', '-parse_seqids', '-mask_data', 'genome_dust.asnb', '-out',
           'genome']
    lib.runSubprocess(cmd, output, lib.log)
    cmd = ['tblastn', '-num_threads', str(cpus), '-db', 'genome', '-query', query, '-max_target_seqs', str(maxhits),
           '-db_soft_mask', '11', '-threshold', '999', '-evalue', '1e-10', '-outfmt', '6', '-out', 'filter.tblastn.tab']
    lib.runSubprocess(cmd, output, lib.log)
...
def runExonerate(input):
    ...
    with open(scaffold, 'w') as output2:
        with open(os.path.join(tmpdir, 'scaffolds', ScaffID+'.fa'), 'rU') as fullscaff:
            for header, Sequence in SimpleFastaParser(fullscaff):
                ...
                start = ScaffStart - 3000  # TODO: change this value?
                if start < 1:
                    start = 1
                end = ScaffEnd + 3000
                if end > len(Sequence):
    ...
    cmd = ['exonerate', '--model', 'p2d', '--showvulgar', 'no', '--showalignment', 'no', '--showquerygff', 'no',
           '--showtargetgff', 'yes', '--percent', '80', '--ryo', ryo , query, scaffold]
