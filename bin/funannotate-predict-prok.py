...
parser.add_argument('--min_protlen', default=20, type=int, help='Minimum amino acid length for valid gene model')
...
-------------------------------------------------------" % (len(longest10), ', '.join([str(x) for x in longest10])))
    ...
    if len(predictions_run) > 1:  # if more than one program was run, use EVM
        ...
        pred_in = [Augustus, GeneMark, Prodigal]
        if args.pasa_gff:
        ...
        Weights = os.path.join(args.out, 'predict_misc', 'weights.evm.txt')
        with open(Weights, 'w') as output:
            output.write("ABINITIO_PREDICTION\tProdigal\t1\n")
